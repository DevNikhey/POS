...
public class CenterFirstController : Controller
{
    public override Task<(int Row, int Col)> GetMove(GameModel model, int player)
    {
        int mid = model.Size / 2;
        if (model.GetCell(mid, mid).Player == 0)
            return Task.FromResult((mid, mid));

        for (int d = 1; d <= mid; d++)
            for (int dr = -d; dr <= d; dr++)
                for (int dc = -d; dc <= d; dc++)
                {
                    int r = mid + dr, c = mid + dc;
                    if (r >= 0 && c >= 0 && r < model.Size && c < model.Size && model.GetCell(r, c).Player == 0)
